...
export class ConversationRepository implements IConversationRepository {
  ...
  async save(userId: UUID, role: 'user' | 'assistant', content: string): Promise<AIConversation> {
  ...
  async getHistory(userId: UUID, limit = 20): Promise<AIConversation[]> {
  ...
  async clearHistory(userId: UUID): Promise<void> {
    await this.db.from('ai_conversations').delete().eq('user_id', userId);
  }
}
